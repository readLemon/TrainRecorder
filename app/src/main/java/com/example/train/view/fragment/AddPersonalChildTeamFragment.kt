 ...
 * on 20-2-29
 */
class AddPersonalChildTeamFragment : BaseFragment() {
    ...
    override fun initial(view: View) {
        ...
        btn_add_personal_fm_submit.setOnClickListener {
            if (isInputAvailable(project, where)) {
                if (isInputAvailable(reason)) {
                    viewModel.addLeave(project = project, time = tempTime, reason = reason)
                } else {
                    viewModel.addAbsent(time = tempTime, project = project)
                }
            } else {
